...
headers on the Webfinger endpoint. So, Webfinger is out and instead we will 
specify a _storage first_ approach that makes more sense to begin with.
...
calls making it easy to see what is going on.
...
A remoteStorage server has an API root directory, for instance:

    https://www.example.org/remoteStorage/api/

This API root directory needs to be followed by the "resource owner", i.e. the 
identifier of the user the files belong to. We call this the *userRoot* 
directory.

    https://www.example.org/remoteStorage/api/john.doe/

So, all calls to either of the above `GET`, `PUT` and `DELETE` MUST be to 
...
To indicate wheter or not a file or directory listing is requested the URL 
...
Before an application can use the `remoteStorage` API it needs to obtain an 
