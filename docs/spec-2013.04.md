...
* `rww_api_uri` - URL pointing to the *userRoot* at the storage server
* `rww_authz_uri` - URL pointing to the OAuth authorize endpoint
* `rww_version` - `2013.04`

The `rww_api_uri` parameter is for example like specified before: 
`https://www.example.org/remoteStorage/api/john.doe/` 
...
A full URL then looks like this:
    ...
    https://todomvc.example.org/#rww_api_uri=https://www.example.org/remoteStorage/api/john.doe/&rww_authz_uri=https://auth.example.org/oauth2/authorize&rww_version=2013.04
