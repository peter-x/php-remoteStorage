# Introduction
After lots of experience with designing the _remoteStorage_ API as part of the
...
implementing the _remoteStorage_ service on embedded devices and mobile 
platforms.
...
meaningful way, i.e.: add _remoteStorage_ information and also provide CORS 
...
operating systems would be able to use _remoteStorage_ directly, but due to 
...
own _remoteStorage_ server for the platform of their choice.

So, even though there is no longer discovery, there still is authorization, 
using a minimal OAuth profile and there is still the _remoteStorage_ API to 
...
There are three HTTP verbs available in the _remoteStorage_ API:
...
of the file with the latest timestamp contained in that directory, all files, 
no matter how deep in the tree below that directory, up to the *userRoot*. 

When implementing _remoteStorage_ on regular file systems it can be quite heavy 
to implement this at the a file directory listing request comes in, so it may 
be beneficial to implement this while storing files using `PUT` requests. At 
that time a _touch_ on all directories containing the updated file up to the 
*userRoot* will take care of this.

This feature is required because that makes it cheap for clients to maintain a 
copy that is in sync with the contents of the storage server without traversing 
the entire storage to locate updated objects.
