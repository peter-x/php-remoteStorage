...
be stored a seperate error needs to be given back to the user. 
...
It is also possible the an application tries to access files or directories 
to which it does not have any permission. For example the user `john.doe` 
authorized an application to access his calendar data, but the application
now tries to access data belonging to `jane.doe`. This should not be allowed
and an error should be returned:
    ...
    {"error":"access_denied","description":"cannot access files belong to other user"}
...
* `storage_root` - the *userRoot* at the storage server
* `authorize_endpoint` - the OAuth authorize endpoint

The `storage_root` parameter is for example like specified before: 
`https://www.example.org/remoteStorage/api/john.doe/` 
while the OAuth authorize endpoint points to the OAuth server, for example: 
